...
class TwitterTweetsDriverAdapterImpl implements TwitterTweetsDriverAdapter {
  // int periodoMeses = 1;
  final List<TweetKyaryEntitie> _listaTweets = [];
  final List<TweetKyaryEntitie> _listaTweetsEnviar = [];
  ...
  // * El modelo (TweetKyaryObjetoModel) esta mal debe recibir todo lo de tweet
  // * No hay ningun helper que transforme del modelo (TweetKyaryObjetoModel)
  // * Se esta pasando directamente a la entidad
  ...
  Future<List<TweetKyaryEntitie>> getTweetsKyary() async {
    ...
    TweetKyaryEntitie tweetAdd = TweetKyaryEntitie(
      textoTweet: '',
      tweetId: '',
      imagenesId: [],
    );
    ...
    for (TweetData tweet in tweetsKyaryData) {
